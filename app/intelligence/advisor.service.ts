...
import { getBusinessMatch } from "./match.engine";
...
export function getAtlasAdvisorRecommendations(
  ...
  if (bestBusiness?.recommended) {
    ...
    recommendations.push({
      ...
      confidence:
        match?.overall ??
        bestBusiness.roiScore,

      match,

      estimatedProfit:
        bestBusiness.estimatedProfit,

      estimatedTimeMinutes: 60,

      href:
        `/data/businesses/${bestBusiness.id}`,
    });
  }
  ...
  if (
    ...
    recommendations.push({
      id:
        "optimize-existing-empire",

      title:
        "Optimize your current empire",
