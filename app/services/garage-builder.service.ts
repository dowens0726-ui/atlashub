...
export type GaragePlayStyle = "balanced" | "racing" | "missions" | "beginner";
...
function getAffordableVehicles(budget: number) {
  return vehicles.filter((vehicle) => vehicle.price <= budget);
}
...
function pickBestVehicle(
  ...
  return candidates
    .filter((vehicle) => !excludedSlugs.includes(vehicle.slug))
    .sort((a, b) => selector(b) - selector(a))[0];
}

export function buildGarage(
  budget: number,
  playStyle: GaragePlayStyle = "balanced"
): GarageBuild {
  const affordableVehicles = getAffordableVehicles(budget);
  ...
  function addRecommendation(
    ...
    selectedSlugs.push(vehicle.slug);
  }

  if (playStyle === "racing") {
    addRecommendation(
      "Performance Pick",
      pickBestVehicle(affordableVehicles, getPerformanceScore, selectedSlugs),
      "Chosen for strong speed, acceleration, handling, and braking."
    );

    addRecommendation(
      "Best Value",
      pickBestVehicle(affordableVehicles, getValueScore, selectedSlugs),
      "Chosen for strong performance relative to cost."
    );
  } else if (playStyle === "missions") {
    addRecommendation(
      "Daily Driver",
      pickBestVehicle(affordableVehicles, getDailyDriverScore, selectedSlugs),
  ...
  } else if (playStyle === "beginner") {
    addRecommendation(
      "Beginner Pick",
      pickBestVehicle(affordableVehicles, getBeginnerScore, selectedSlugs),
  ...
  const totalCost = recommendations.reduce(
    (sum, recommendation) => sum + recommendation.vehicle.price,
    0
  );
    ...
    recommendations.length === 0
      ...
      : Math.round(
          recommendations.reduce(
            (sum, recommendation) =>
              sum + getAtlasVehicleScore(recommendation.vehicle).overall,
            0
